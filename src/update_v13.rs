...
pub fn run(config: &Config, status: &Arc<Mutex<String>>) -> Result<String, String> {
    ...
    set_phase(
        status,
        config.language,
        "Baixando release (retomável)...",
        "Downloading release (resumable)...",
    );
        ...
        if let Err(health_error) = restart::execute(&config.health_check_command) {
            if !old.is_empty() && old != plan.release.tag_name {
                ...
                let result = if rollback_result.is_ok() {
                    ...
                    "health-check-failed; automatic-rollback-failed"
                };
    ...
    let (deleted, failed) = install::prune_old_releases(
    ...
    if !failed.is_empty() {
        message.push_str(&format!(
            "\n{}: {}",
            tr(config.language, "Aviso de limpeza", "Cleanup warning"),
            failed
                .iter()
                .map(|(t, e)| format!("{t}: {e}"))
                .collect::<Vec<_>>()
