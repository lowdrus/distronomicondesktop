use crate::{config::{tr, Config}, install, lock, release, restart, state};
...
pub fn rollback(config: &Config) -> Result<String, String> {
    ...
    let current = install::current_tag(&install_root, &config.app_name)
        .map_err(|e| e.to_string())?
        .ok_or_else(|| tr(config.language, "Nenhuma versão instalada para rollback.", "No installed version available for rollback.").to_string())?;
    let releases = install::list_releases(&install_root, &config.app_name).map_err(|e| e.to_string())?;
    let previous = releases.into_iter().find(|tag| tag != &current)
        .ok_or_else(|| tr(config.language, "Nenhuma versão anterior disponível.", "No previous version is available.").to_string())?;

    install::activate_release(&install_root, &config.app_name, &previous).map_err(|e| e.to_string())?;
    let state_path = PathBuf::from(&config.state_root).join(&config.app_name).join("state.json");
    ...
    if !config.restart_command.is_empty() {
        restart::execute(&config.restart_command)
            .map_err(|e| format!("{}: {e}", tr(config.language, "Rollback concluído, mas o reinício falhou", "Rollback completed, but restart failed")))?;
    }

    Ok(format!("{}: {current} → {previous}", tr(config.language, "Rollback concluído", "Rollback completed")))
}
...
pub fn doctor(config: &Config) -> Result<String, String> {
    ...
    let state_path = PathBuf::from(&config.state_root).join(&config.app_name).join("state.json");
    let current = install::current_tag(&install_root, &config.app_name).map_err(|e| e.to_string())?;
    let releases = install::list_releases(&install_root, &config.app_name).map_err(|e| e.to_string())?;
    let state_value = state::load(&state_path).map_err(|e| e.to_string())?;
    let mut problems = Vec::new();

    match &current {
        Some(tag) if !releases_dir.join(tag).is_dir() => problems.push(tr(config.language,
            "current.txt aponta para uma release ausente", "current.txt points to a missing release")),
        None => problems.push(tr(config.language, "Nenhuma versão atual registrada", "No current version is recorded")),
        _ => {}
    }
    if current.is_some() && !bin_dir.is_dir() {
        problems.push(tr(config.language, "Diretório bin ausente", "bin directory is missing"));
    }
    if bin_dir.is_dir() && fs::read_dir(&bin_dir).map_err(|e| e.to_string())?.next().is_none() {
        problems.push(tr(config.language, "Diretório bin está vazio", "bin directory is empty"));
    }
    if let (Some(current_tag), Some(saved)) = (&current, &state_value)
        && !saved.latest_tag.is_empty() && saved.latest_tag != *current_tag {
        problems.push(tr(config.language, "state.json e current.txt estão divergentes", "state.json and current.txt disagree"));
    }

    if problems.is_empty() {
        Ok(format!("{}\n{}: {}\n{}: {}",
            tr(config.language, "Diagnóstico OK.", "Doctor OK."),
            tr(config.language, "Versão atual", "Current version"), current.unwrap_or_else(|| "-".into()),
            tr(config.language, "Releases instaladas", "Installed releases"), releases.len()))
    } else {
        Ok(format!("{}\n- {}", tr(config.language, "Diagnóstico encontrou problemas:", "Doctor found problems:"), problems.join("\n- ")))
    }
}
