use crate::{config::{tr, Config, Language}, install, lock, release, restart, state, verify};
...
pub fn run(config: &Config, status: &Arc<Mutex<String>>) -> Result<String, String> {
    ...
    let _guard = lock::acquire(&lock_path, Duration::from_secs(30))
        .map_err(|e| format!("{}: {e}", tr(config.language, "Não foi possível adquirir o lock", "Could not acquire update lock")))?;

    set_phase(status, config.language, "Consultando releases no GitHub...", "Checking GitHub releases...");
    ...
    if fetched.not_modified {
        return current.map(|tag| format!("{}: {tag}", tr(config.language, "Já está atualizado", "Already up to date")))
    ...
    let temp_path = temp_dir.join(format!("{}-{}", safe_name(&latest.tag_name), safe_name(&asset.name)));

    let result = (|| -> Result<String, String> {
        set_phase(status, config.language, "Baixando release...", "Downloading release...");
