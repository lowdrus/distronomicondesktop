...
pub fn backup_configurable_files(config: &Config) -> Result<Option<PathBuf>, String> {
    ...
    let bin = PathBuf::from(&config.install_root)
        .join(&config.app_name)
        .join("bin");
    for item in items {
        ...
        if !source.exists() {
            log(
                config,
                "WARNING",
                &format!("Backup item not found: {}", source.display()),
            );
            continue;
        }
        let name = source
    ...
    log(
        config,
        "INFO",
        &format!("Backup created at {}", backup_root.display()),
    );
    Ok(Some(backup_root))
}
...
pub fn recover_state(config: &Config) -> Result<String, String> {
    ...
    if state::load(&state_path).is_ok_and(|v| v.is_some()) {
        return Ok(tr(
            config.language,
            "Estado válido; nenhuma recuperação necessária.",
            "State is valid; no recovery is required.",
        )
        .into());
    }
    ...
    let current = install::current_tag(&install_root, &config.app_name)
        ...
        .or_else(|| {
            install::list_releases(&install_root, &config.app_name)
                .ok()
        ...
        .ok_or_else(|| {
            tr(
                config.language,
                "Nenhuma release instalada foi encontrada para reconstruir o estado.",
                "No installed release was found to rebuild state.",
            )
            .to_string()
        })?;
    ...
    log(
        config,
        "WARNING",
        &format!("State rebuilt from releases/current.txt: {current}"),
    );
    Ok(format!(
        "{}: {current}",
        tr(config.language, "Estado reconstruído", "State rebuilt")
    ))
}
...
pub fn verify_authenticode_tree(config: &Config, root: &Path) -> Result<String, String> {
    ...
    for exe in exes {
        match authenticode_status(&exe)? {
            ...
            status => {
                return Err(format!(
                    "Authenticode validation failed for {}: {status}",
                    exe.display()
                ));
            }
        }
    ...
    Ok(if signed == 0 {
        "No signed executable found".into()
    } else {
        format!("Authenticode valid for {signed} executable(s)")
    })
}
...
fn authenticode_status(path: &Path) -> Result<String, String> {
    #[cfg(windows)]
    {
        ...
        let script =
            format!("(Get-AuthenticodeSignature -LiteralPath '{escaped}').Status.ToString()");
...
pub fn prune_policy(config: &Config, current_tag: &str) -> Result<Vec<String>, String> {
    ...
    for (index, entry) in entries.into_iter().enumerate() {
        ...
        if entry.0 != current_tag && (too_old || over_count) {
            if fs::remove_dir_all(&entry.1).is_ok() {
