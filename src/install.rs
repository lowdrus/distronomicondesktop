use std::{fs, io::{self, Cursor, Read, Write}, path::{Path, PathBuf}, time::{SystemTime, UNIX_EPOCH}};
...
pub fn install_release(
    ...
    let result = if asset_name.to_ascii_lowercase().ends_with(".zip") {
        ...
        fs::write(staging.join(asset_name), bytes).map_err(|e| e.to_string())
    };
...
fn extract_zip(bytes: &[u8], destination: &Path) -> Result<(), String> {
    ...
    for index in 0..archive.len() {
        ...
        io::copy(&mut entry.take(MAX_FILE_BYTES + 1), &mut output).map_err(|e| e.to_string())?;
