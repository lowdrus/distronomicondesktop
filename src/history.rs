...
impl HistoryEntry {
    pub fn new(action: &str, from_version: &str, to_version: &str, asset: &str, sha256: &str, result: &str) -> Self {
        Self {
...
pub fn append(path: &Path, entry: HistoryEntry) -> io::Result<()> {
    ...
    items.push(entry);
    if items.len() > 500 { items.drain(0..items.len()-500); }
    save(path, &items)
}

fn save(path: &Path, items: &[HistoryEntry]) -> io::Result<()> {
    let parent = path.parent().ok_or_else(|| io::Error::new(io::ErrorKind::InvalidInput, "history path has no parent"))?;
    fs::create_dir_all(parent)?;
    let tmp = path.with_extension("json.tmp");
    let backup = path.with_extension("json.bak");
    let bytes = serde_json::to_vec_pretty(items).map_err(|e| io::Error::new(io::ErrorKind::InvalidData, e))?;
    {
        use std::io::Write;
...
pub fn format_recent(path: &Path, limit: usize) -> io::Result<String> {
    ...
    if out.is_empty() { out.push_str("No history entries.\n"); }
    Ok(out)
}
