...
pub fn fetch_tag(client: &Client, host: &str, repo: &str, token: Option<&str>, tag: &str) -> Result<FetchResult, String> {
    let host = host.trim_end_matches('/');
    let url = format!("{host}/repos/{repo}/releases/tags/{tag}");
    let mut request = client.get(url).header(ACCEPT, "application/vnd.github+json").header("X-GitHub-Api-Version", "2022-11-28");
...
pub fn fetch_latest(
    ...
    let host = host.trim_end_matches('/');
    let url = if allow_prerelease { format!("{host}/repos/{repo}/releases") } else { format!("{host}/repos/{repo}/releases/latest") };

    let mut request = client.get(url).header(ACCEPT, "application/vnd.github+json").header("X-GitHub-Api-Version", "2022-11-28");
    if let Some(token) = token.filter(|t| !t.trim().is_empty()) { request = request.header(AUTHORIZATION, format!("Bearer {}", token.trim())); }
