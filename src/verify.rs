...
use std::{fs::File, io::{self, Read}, path::Path, thread, time::Duration};
...
fn send_with_retry(client: &Client, token: Option<&str>, asset: &Asset) -> Result<reqwest::blocking::Response, String> {
...
pub fn download_asset_to_file(client: &Client, token: Option<&str>, asset: &Asset, destination: &Path) -> Result<(), String> {
...
pub fn verify_sha256(client: &Client, token: Option<&str>, asset_name: &str, downloaded_path: &Path, checksum_asset: &Asset) -> Result<(), String> {
    let text = send_with_retry(client, token, checksum_asset)?.text().map_err(|e| e.to_string())?;
    ...
    else { Err(format!("SHA-256 mismatch for {asset_name}: expected {expected}, got {actual}")) }
}
