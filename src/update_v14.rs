...
pub fn run(
    ...
    set_phase(
        status,
        config.language,
        "Testando conectividade e planejando...",
        "Testing connectivity and planning...",
    );
    ...
    if !config.allow_downgrade
        && !old.is_empty()
        && features_v14::is_downgrade(&old, &plan.release.tag_name)
    {
        let error = format!(
            "{}: {} → {}",
            tr(
                config.language,
                "Downgrade bloqueado. Habilite a autorização explícita no perfil para continuar",
                "Downgrade blocked. Enable explicit authorization in the profile to continue"
            ),
            old,
            plan.release.tag_name
        );
    ...
    features_v14::log(
        ...
        "INFO",
        &format!(
            "Space check: required={}, available={}",
            features_v14::format_size(required),
            features_v14::format_size(available)
        ),
    );
    ...
    set_phase(
        status,
        config.language,
        "Criando backup dos arquivos configuráveis...",
        "Backing up configurable files...",
    );
    ...
    let temp_path = temp_dir.join(format!(
        "{}-{}",
        safe_name(&plan.release.tag_name),
        safe_name(&plan.asset.name)
    ));

    set_phase(
        status,
        config.language,
        "Baixando release (retomável e cancelável)...",
        "Downloading release (resumable and cancellable)...",
    );
    let resumed = match download_resume::download_cancellable(
        &client,
        token(config),
        &plan.asset,
        &temp_path,
        ...
        Err(error) if error == "DOWNLOAD_CANCELLED" => {
            features_v14::log(
                config,
                "WARNING",
                "Download cancelled by user; active installation was not changed",
            );
            return Err(tr(
                config.language,
                "Download cancelado. A instalação atual permaneceu intacta.",
                "Download cancelled. The current installation was left intact.",
            )
            .into());
        }
    ...
    set_phase(
        status,
        config.language,
        "Validando assinatura Authenticode...",
        "Validating Authenticode signature...",
    );
    ...
    let mut state_value = state::load(&state_path)
    ...
    let mut message = format!(
        ...
        tr(
            config.language,
            "Espaço estimado usado no planejamento",
            "Estimated space used in planning"
        ),
        features_v14::format_size(required)
    );
    if let Some(path) = backup {
        message.push_str(&format!(
            "\n{}: {}",
            tr(config.language, "Backup", "Backup"),
            path.display()
        ));
    }
    if resumed {
        message.push_str(&format!(
            ...
            tr(
                config.language,
                "Download retomado de uma transferência parcial.",
                "Download resumed from a partial transfer."
            )
        ));
    }
    if !deleted.is_empty() {
        message.push_str(&format!(
            ...
            tr(
                config.language,
                "Versões removidas pela política de retenção",
                "Releases removed by retention policy"
            ),
            deleted.join(", ")
        ));
    }
